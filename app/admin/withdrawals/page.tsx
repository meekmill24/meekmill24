...
import { Check, X, Search, ArrowUpFromLine, Clock, Wallet, User as UserIcon, AlertTriangle, Loader2, Layers, XCircle, Users } from 'lucide-react'; 
    ...
    <div className="space-y-8 animate-in fade-in duration-500"> 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:border-rose-500/30 transition-all">
          ...
          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Total Disbursements</p>
          <h3 className="text-3xl font-black text-white italic tracking-tighter">{withdrawals.length}</h3>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:border-emerald-500/30 transition-all">
          ...
          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Successfully Disbursed</p>
          <h3 className="text-3xl font-black text-emerald-500 italic tracking-tighter">{withdrawals.filter(w => w.status === 'approved').length}</h3>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:border-blue-500/30 transition-all">
          ...
          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Awaiting Transmission</p>
          <h3 className="text-3xl font-black text-blue-500 italic tracking-tighter">{withdrawals.filter(w => w.status === 'pending').length}</h3>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-6 rounded-[32px] backdrop-blur-md relative overflow-hidden group hover:border-rose-500/30 transition-all">
          ...
          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Rejected Requests</p>
          <h3 className="text-3xl font-black text-rose-500 italic tracking-tighter">{withdrawals.filter(w => w.status === 'rejected').length}</h3>
        </div>
      </div>

      <div className="bg-slate-900/40 border border-slate-800 rounded-[32px] backdrop-blur-md overflow-hidden relative">
          ...
          <table className="w-full text-sm border-separate border-spacing-0" style={{ minWidth: '2500px', tableLayout: 'auto' }}>
            <thead className="sticky top-0 z-30">
              <tr className="text-left text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-800 bg-slate-900/95 backdrop-blur-md">
                ...
                <th className="px-4 md:px-8 py-6 text-right border-l border-white/5 opacity-50 font-bold uppercase tracking-widest text-[9px]">Approval Cycle</th>
              </tr>
            </thead>
