              ...
              body: JSON.stringify({
                  ...
                  html: `
                      <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 40px; background: #0a0a0b; color: white; border-radius: 20px;">
                          <h2 style="color: ${status === 'approved' ? '#10b981' : '#f43f5e'}; text-transform: uppercase; font-style: italic;">Deposit ${status}</h2>
                          <p>Hello <strong>${profile.username}</strong>,</p>
                          <p>Your institutional funding request of <strong>$${amount.toLocaleString()}</strong> has been <strong>${status}</strong>.</p>
                          ${status === 'approved' ? '<p>Your balance has been updated and is ready for strategic optimization.</p>' : '<p>Please contact governance support for more details regarding this rejection.</p>'}
