...
export default function SecurityPage() {
    ...
    const handleSavePin = async () => {
        if (newPin.length !== 6) {
            setPinMatchError("PIN MUST BE 6 DIGITS");
            return;
        }
        if (newPin !== confirmPin) {
            setPinMatchError("SEQUENCE MISMATCH");
            return;
        }
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 pb-40 space-y-12 animate-in fade-in duration-700">
            ...
            {/* Critical Action Node - Withdrawal PIN */}
            <div 
                onClick={() => isVerified && setShowPinModal(true)}
                className={`glass-card-strong p-10 rounded-[48px] border border-rose-500/20 bg-rose-500/5 group hover:bg-rose-500/10 transition-all cursor-pointer shadow-xl ${!isVerified ? 'opacity-40 grayscale pointer-events-none' : ''}`}
            >
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex items-center gap-6">
                        <div className="w-14 h-14 rounded-2xl bg-rose-500/10 flex items-center justify-center text-rose-500 border border-rose-500/20 shadow-lg">
                            <Key size={28} />
                        </div>
                        <div className="text-center md:text-left space-y-1">
                            <h4 className="text-xl font-black text-white italic uppercase tracking-tight italic">Withdrawal Sequence</h4>
                            <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest italic opacity-60">
                                {profile?.withdrawal_password ? 'RECALIBRATE WITHDRAWAL PIN' : 'INITIALIZE WITHDRAWAL PIN'}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] italic group-hover:text-white transition-colors">Governance Layer</span>
                        <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-rose-500/20 transition-all shadow-inner">
            ...
            <div className="p-8 rounded-[40px] bg-slate-900/40 border border-white/5 flex items-center justify-center gap-4 text-center">
                ...
                <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] italic opacity-80 leading-relaxed italic">
            ...
            {showPinModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
                    <div className="absolute inset-0 bg-black/90 backdrop-blur-md" onClick={() => !isLoading && setShowPinModal(false)} />
                    <div className="relative w-full max-w-lg bg-[#0a0a0a] border border-white/10 rounded-[48px] p-10 lg:p-14 shadow-[0_0_100px_rgba(244,63,94,0.15)] animate-in zoom-in duration-300">
                        <div className="flex flex-col items-center mb-10">
                            <div className="w-20 h-20 rounded-[30px] bg-rose-500/10 flex items-center justify-center mb-6 border border-rose-500/20 shadow-inner">
                            ...
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mt-2 opacity-60">Authorize Payout PIN</p>
                        </div>

                        <div className="space-y-8">
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2 block opacity-40">Define 6-Digit Sequence</label>
                                <div className="relative">
                                    <input 
                                        ...
                                        placeholder="••••••"
                                        className="w-full bg-white/[0.03] border border-white/10 rounded-[28px] py-8 px-10 text-3xl font-black text-white outline-none focus:border-rose-500/40 transition-all text-center tracking-[0.5em] font-mono shadow-inner"
                                    />
                                    {pinMatchError && <p className="text-[9px] font-black text-rose-500 uppercase text-center mt-3 tracking-widest">{pinMatchError}</p>}
                                </div>
                            </div>
                            
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2 block opacity-40">Confirm Sequence</label>
                                <input 
                                    type="password"
                                    maxLength={6}
                                    value={confirmPin}
                        ...
                        <div className="mt-12 space-y-4">
                            <button 
                                ...
                                className="w-full h-20 bg-rose-600 text-white rounded-[32px] font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-rose-900/30 active:scale-[0.98] transition-all disabled:opacity-20 flex items-center justify-center gap-3"
                            >
                                {isLoading ? <Loader2 size={24} className="animate-spin" /> : <>Synchronize Key <ShieldCheck size={20} /></>}
                            </button>
                            <button 
